// Garage İstanbul Müşteri Memnuniyet Anketi
...
const contactAllowed = document.getElementById("contactAllowed");
...
function renderQuestions(){
  questionsEl.innerHTML = questions.map((q,index)=>{
    ...
    const ratings=[1,2,3,4,5].map(value=>`
      <input required type="radio" name="q${no}" id="q${no}_${value}" value="${value}">
      <label for="q${no}_${value}"><span>${emojis[value-1]}</span><small>${value}</small></label>
    `).join("");
    return `<div class="question"><div class="question-title">${no}. ${q}</div><div class="rating">${ratings}</div></div>`;
  }).join("");
}
